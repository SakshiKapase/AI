...
Difference in vertical + difference in horizontal i.e Manhattan Distance(|x1-x2|+|y1-y2|)
*/
...
int main(){
    int start[9];
    int goal[9];
    cout << "Enter the start state:(Enter -1 for empty): "<<endl;
    for(int i = 0; i < 9; i++){
        cin >> start[i];
    }
    cout << "Enter the goal state:(Enter -1 for empty): "<<endl;
    for(int i = 0; i < 9; i++){
        cin >> goal[i];
    }
